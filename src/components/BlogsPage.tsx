...
const BlogNewsletterForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await submitForm({ name: name.trim(), email: email.trim(), formType: 'newsletter' });
    if (result.success) {
      setEmail('');
      setName('');
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto space-y-4">
      ...
      {submitStatus === 'success' && <p className="text-green-400 text-sm text-center">✅ Successfully subscribed!</p>}
      {submitStatus === 'error' && <p className="text-red-400 text-sm text-center">❌ Something went wrong. Please try again.</p>}
    </form>
  );
};
...
const stripMarkdown = (md: string): string =>
  md.replace(/<!--([\s\S]*?)-->/g, '').replace(/!\[.*?\]\(.*?\)/g, '').replace(/\[(.*?)\]\(.*?\)/g, '$1').replace(/[#_*>`~-]+/g, '').replace(/\s+/g, ' ').trim();
...
const useIntersectionObserver = (options: IntersectionObserverInit = {}): [React.RefObject<HTMLDivElement>, boolean] => {
  ...
  const memoizedOptions = useMemo(() => ({ threshold: 0.1, ...options }), [options]);
...
// Blog Card
const BlogCard: React.FC<{ title: string; category?: string; image?: string; delay?: number; onReadMore?: () => void }> = ({
  title,
  category = 'Article',
    ...
    >
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <span className="bg-black backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-medium border border-white/20">{category}</span>
        </div>
      </div>
      <div className="p-6">
        <h3 className="text-xl font-bold text-white mb-3 leading-tight group-hover:text-purple-400 transition-colors">{title}</h3>
        <div className="flex items-center text-purple-400 font-medium text-sm group-hover:text-purple-300 cursor-pointer transition-colors">
          <span>Read More</span>
...
// Featured Blog Card
const FeaturedBlogCard: React.FC<{ title: string; category?: string; image?: string; excerpt?: string; onReadMore?: () => void }> = ({
  title,
  category = 'Article',
  image = '/placeholder.svg',
  excerpt,
  ...
  return (
    <div
      ref={ref}
      className={`col-span-2 bg-black backdrop-blur-sm border border-purple-300/20 rounded-xl overflow-hidden hover:shadow-2xl hover:border-purple-300/40 transition-all duration-500 cursor-pointer w-[95%] mx-auto ${
        hasIntersected ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
      }`}
      onClick={onReadMore}
    >
      <div className="grid md:grid-cols-2 h-full">
        ...
        <div className="p-8 flex flex-col justify-center">
          <h3 className="text-2xl font-bold text-white mb-4 leading-tight group-hover:text-purple-400 transition-colors">{title}</h3>
          {excerpt && <p className="text-gray-300 mb-6 leading-relaxed">{excerpt}</p>}
          <div className="flex items-center text-purple-400 font-medium group-hover:text-purple-300 cursor-pointer" onClick={onReadMore}>
...
const BlogPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadPosts = async () => {
      try {
        setLoading(true);
        setError(null);
        if (!blogsMeta || blogsMeta.length === 0) return setError('No blog metadata found');
        const modules = import.meta.glob('../blogs/*.md', { as: 'raw', eager: true }) as Record<string, string>;
        if (!modules || Object.keys(modules).length === 0) return setError('No markdown files found');

        const loadedPosts = blogsMeta.map((meta) => {
          const fileKey = Object.keys(modules).find((key) => key.split('/').pop()?.replace('.md', '') === meta.slug);
          const raw = fileKey ? modules[fileKey] : '';
          const { content, data } = processMarkdownContent(raw);
          return {
            ...meta,
            date: meta.date || (data.date as string),
  ...
  useEffect(() => {
    ...
    const handleHashChange = () => {
      ...
      if (!hash) return setSelectedPost(null);
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => window.location.reload()} className="bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded">
            Retry
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="container mx-auto px-12 py-16">
        ...
        >
          ...
          {/* Hero Heading */}
          <h1
            ref={heroHeading.ref as React.RefObject<HTMLHeadingElement> | null}
            className={`text-5xl md:text-6xl font-bold mb-6 leading-tight ${
              heroHeading.animationClasses || ''
            }`}
          >
          ...
          {/* Hero Subheading */}
          <p
            ref={heroSubheading.ref as React.RefObject<HTMLParagraphElement> | null}
            className={`text-gray-300 text-lg max-w-2xl mx-auto leading-relaxed ${
              heroSubheading.animationClasses || ''
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {posts.length > 0 && <FeaturedBlogCard {...posts[0]} onReadMore={() => setSelectedPost(posts[0])} />}
          {posts.slice(1).map((post, i) => (
            <BlogCard key={post.slug || i} {...post} delay={i * 100} onReadMore={() => setSelectedPost(post)} />
          ))}
        </div>
        ...
        <div className="mt-20 text-center">
          <div className="bg-black backdrop-blur-sm border border-purple-300/20 rounded-2xl p-12 max-w-2xl mx-auto">
            ...
            <h3
              ref={newsletterHeading.ref as React.RefObject<HTMLHeadingElement>}
              className={`text-2xl font-bold mb-4 ${newsletterHeading.animationClasses}`}
            >
              Stay Updated
            </h3>
            <p className="text-gray-300 mb-8">Get the latest AI insights delivered straight to your inbox. No spam, just valuable content.</p>
            <BlogNewsletterForm />
          </div>
        </div>
      </div>
